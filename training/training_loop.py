    ...
    while True:
        ...
        for phase, phase_gen_z, phase_gen_c in zip(phases, all_gen_z, all_gen_c):
            ...
            for round_idx, (real_img, mask, real_c, gen_z, gen_c) in enumerate(zip(phase_real_img, phase_mask, phase_real_c, phase_gen_z, phase_gen_c)):
                ...
                gain = phase.interval
                loss.accumulate_gradients(phase=phase.name, real_img=real_img, mask=mask, real_c=real_c, real_pick=None, gen_z=gen_z, gen_c=gen_c, sync=sync, gain=gain)
...
def finetune_loop(
    ...
    name, module, opt_kwargs, reg_interval = 'G', G, G_opt_kwargs, G_reg_interval

    filter_list = ['synthesis.first_stage', 'synthesis.first_stage.dec_conv.0']
    ft_params = []
    for pname, param in module.named_parameters():
        flag = False
        for fname in filter_list:
    ...
    opt_kwargs.lr = opt_kwargs.lr * 0.1
    opt = dnnlib.util.construct_class_by_name(params=ft_params, **opt_kwargs)
    phases += [dnnlib.EasyDict(name=name+'finetune', module=module, opt=opt, interval=1)]
        ...
        for phase, phase_gen_z, phase_gen_c in zip(phases, all_gen_z, all_gen_c):
            ...
            with torch.autograd.profiler.record_function(phase.name + '_opt'):
                for name, param in phase.module.named_parameters():
                    ...
                    '''print(layer_name)
                    if layer_name == 'synthesis.first_stage' or layer_name == 'synthesis.first_stage.dec_conv.0':
                        print('should be train: ' + layer_name)'''
                phase.opt.step()
